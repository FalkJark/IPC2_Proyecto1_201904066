...
def crear_tablero(matriz):
    ...
    file.write('digraph tablero_actual{\n')
    ...
    for i in range(ejey.dimension):
        if i == 0:   
            file.write('f0 [label = "0"  pos = "5.3,3.5!",width = 1.5, style = filled, fillcolor = lightpink, group = 1 ];\n')
            heady = heady.abajo
        else:
            file.write('    f'+str(i)+' [label = "'+str(heady.coordenaday)+'" ,  width = 1.5, style = filled, fillcolor = lightpink, group = 1 ];\n')
            heady = heady.abajo
    ...
    for i in range(ejex.dimension):
        file.write('    c'+str(i)+' [label = "'+str(headx.coordenadax)+'",   width = 1.5, style = filled, fillcolor = lightpink, group ='+str(2+i)+'];\n')
        headx = headx.derecha
    ...
    file.close()
     ...
     #crear archivo pdf
    cmd('dot -Tgif tablero.dot -o tablero.gif')

    #abrir pdf con el navegador
    #webbrowser.open('tablero.gif')
